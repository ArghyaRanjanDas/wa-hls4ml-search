...
for RF in 1 4 8; do
  ...
  python iter_manager_catapult.py -o "$OUT" \
    ...
    --gen_model_config_json config_dense_2layers_size64_bw6_10_14_A.json \
    "${COMMON_ARGS[@]}"

  echo "--- Sub-run B: input=4-32, layer1=64, layer2=4-64 (540 designs) ---"
  python iter_manager_catapult.py -o "$OUT" \
    --flow_config_json config_catapult_flow_rf${RF}.json \
    --gen_model_config_json config_dense_2layers_size64_bw6_10_14_B.json \
    "${COMMON_ARGS[@]}"

  echo "--- Sub-run C: input=4-32, layer1=4-32, layer2=64 (432 designs) ---"
  python iter_manager_catapult.py -o "$OUT" \
    --flow_config_json config_catapult_flow_rf${RF}.json \
    --gen_model_config_json config_dense_2layers_size64_bw6_10_14_C.json \
    "${COMMON_ARGS[@]}"
done
