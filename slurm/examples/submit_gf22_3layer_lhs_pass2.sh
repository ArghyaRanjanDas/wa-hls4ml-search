#!/bin/bash
#SBATCH --job-name=gf22_3layer_lhs_p2
...
#
# GF22nm synthesis for 3-layer dense networks selected by LHS — pass 2.
# N=10,000 samples; RF=1, 4, 8, 16; uses --exclude to avoid pass-1 designs.
# Each RF group uses 3 nodes × 100 parallel slots (300 licenses).
#
# Usage:
#   sbatch slurm/examples/submit_gf22_3layer_lhs_pass2.sh
#
# Optional env vars:
#   CANDIDATES   path to candidates file (default: auto-generated, N_LHS designs)
#   N_LHS        number of LHS samples (default: 10000)
...
N_LHS="${N_LHS:-10000}"
CANDIDATES="${CANDIDATES:-${ARCHIVE_GF22NM}/gf22_lhs_3layer_${N_LHS}.txt}"
EXCLUDE_PASS1="${ARCHIVE_GF22NM}/gf22_lhs_3layer_500.txt"

PARALLELISM=100   # per node; 3 nodes submitted per RF group → 300 total (= all licenses)
SLURM_TIME=05:30:00
...
if [ ! -f "$CANDIDATES" ]; then
    echo "=== Generating LHS candidates (N=${N_LHS}, 3-layer, excluding pass-1) ==="
    python3 "${REPO_DIR}/slurm/examples/sample_lhs_from_archive.py" \
        --layers 3 --n "$N_LHS" \
        --exclude "$EXCLUDE_PASS1" \
        --out "$CANDIDATES"
else
...
run_gf22_group() {
    ...
    local BASE="${SCRATCH}/catapult_gf22_3layer_lhs_${rf_label}"

    echo ""
    echo "=== GF22nm 3-layer LHS  RF=${rf_label} ==="
...
PYEOF
    ...
    # ── Generate three synthesis batch scripts (one per node) ─────────────────
    _make_synth_script() {
        local part="$1" jl="$2"
...
#!/bin/bash
#SBATCH --job-name=gf22_3l_${rf_label}_${part}
...
SBATCH_EOF
    ...
    echo "  Done: gf22 3-layer LHS ${rf_label}."
}
...
echo ""
echo "GF22nm 3-layer LHS pass 2 sweep complete."
