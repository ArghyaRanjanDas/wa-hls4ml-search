#!/bin/bash
#SBATCH --job-name=gf22_3layer_lhs
...
#
# GF22nm synthesis for 3-layer dense networks selected by LHS from the 45nm archive.
# Runs RF=1 and RF=16; each group uses 3 nodes × 100 parallel slots (300 licenses).
#
# Usage:
#   sbatch slurm/examples/submit_gf22_3layer_lhs.sh
#
# Optional env vars:
#   CANDIDATES   path to candidates file (default: auto-generated, N_LHS designs)
#   N_LHS        number of LHS samples (default: 500)
...
if [ ! -f "$CANDIDATES" ]; then
    echo "=== Generating LHS candidates (N=${N_LHS}, 3-layer) ==="
    python3 "${REPO_DIR}/slurm/examples/sample_lhs_from_archive.py" \
        --layers 3 --n "$N_LHS" --out "$CANDIDATES"
else
...
echo ""
echo "GF22nm 3-layer LHS sweep complete."
