...
#
# Nangate 45nm full-cartesian sweep for 1-layer dense networks with 128-neuron coverage.
# Synthesises only the 198 new architectures where input=128 OR layer=128
# (the ≤64 designs are already archived from the existing 1-layer cartesian).
# 198 unique archs × 4 RF = 792 total designs.
# Archives flat to nangate45/.
# RF=1, 4, 8, 16; each group uses 3 nodes × 100 parallel slots (300 licenses).
#
# Usage:
#   sbatch slurm/examples/submit_45nm_1layer_sz128.sh
...
CANDIDATES="${CANDIDATES:-${ARCHIVE_45NM}/nangate45_1layer_sz128_cartesian.txt}"
...
if [ ! -f "$CANDIDATES" ]; then
    echo "=== Cartesian enumeration: 1-layer with 128, new designs only ==="
    python3 - <<PYEOF
import os
...
configs = [
    (inp, l1, bw, act)
    for inp in SIZES for l1 in SIZES
    for bw  in BWS   for act in ACTS
    if inp == 128 or l1 == 128   # skip designs already in the ≤64 cartesian
]
...
with open(out_path, 'w') as f:
    for idx, (inp, l1, bw, act) in enumerate(configs):
        stem  = f"dense_1l_sz128_{idx}"
        parts = [stem, str(inp), str(l1), str(bw), act]
        f.write('\t'.join(parts) + '\n')
...
run_45nm_group() {
    ...
    local BASE="${SCRATCH}/catapult_45nm_1layer_sz128_${rf_label}"

    echo ""
    echo "=== Nangate 45nm 1-layer sz128 cartesian  RF=${rf_label} ==="
...
for i, line in enumerate(lines):
    ...
    try:
        ...
        model = _build_dense_model([(l1_sz, act)], bw, config_params, input_size=in_sz)
...
#!/bin/bash
#SBATCH --job-name=45nm_1l_sz128_${rf_label}_${part}
...
SBATCH_EOF
    ...
    echo "  Done: 45nm 1-layer sz128 ${rf_label}."
}
...
echo ""
echo "Nangate 45nm 1-layer sz128 cartesian sweep complete."
