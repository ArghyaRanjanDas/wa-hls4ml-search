#!/bin/bash
# Full Cartesian product run — all 240 unique single-layer dense NN configs
# (input 4-32, output 4-32, relu/tanh/sigmoid, bitwidth 4-12 even).
# No random sampling.
#
...
python iter_manager_catapult.py \
  ...
  --gen_model_config_json config_dense_single_layer.json          $(: single-layer: in/out 4-32, relu/tanh/sigmoid, 4-12-bit) \
  --cartesian                                                     $(: enumerate all 375 unique configs, no random sampling) \
  --slurm --slurm-qos express_amsc --slurm-time 20:00:00          $(: SLURM array, express_amsc QoS) \
