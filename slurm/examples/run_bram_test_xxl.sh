#!/bin/bash
# BramFactor smoke test — one XXL model (2 layers, 512 neurons, relu, 8-bit).
...
#
# Run from repo root: bash slurm/examples/run_bram_test_xxl.sh
...
python iter_manager_catapult.py \
  ...
  --gen_model_config_json configs/model_sweeps/config_bram_test_xxl.json              $(: fixed model: 2 dense layers, 512 neurons, relu, 8-bit) \
