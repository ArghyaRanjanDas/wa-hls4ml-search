...
- **`express_amsc` QoS cap**: `GrpTRES=node=32` — caps concurrent array tasks at 32 nodes total across the AmSC group. Observed during 100-task scale test (2026-05-01): only 32 ran simultaneously despite licenses being available.
- **Each task uses 1 whole node** (`#SBATCH --nodes=1 --ntasks=1 --cpus-per-task=8 --mem=64G`) — that's 8 cores out of 128 and 64 GB out of 512 GB on the node. ~94% of node resources idle per task. Possible future optimization: outer-array + inner-GNU-parallel pattern to pack ~12 Catapult instances per node (Giuseppe has a reference at `Perlmutter_scripts/perlmutter_slurm/07_array_parallel/`) — deferred pending reviewer guidance.
